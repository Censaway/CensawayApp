...
export const ProcessSelectorModal: React.FC<Props> = ({
  isOpen,
  onClose,
  onSelect,
}) => {
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto min-h-0 pr-1 scrollbar-thin space-y-1">
          {loading && processes.length === 0 ? (
            <div className="flex justify-center py-8 text-gray-500 text-xs">
              Loading...
            </div>
          ) : filtered.length === 0 ? (
            <div className="flex justify-center py-8 text-gray-500 text-xs">
        ...
        <div className="mt-4 pt-4 border-t border-white/5 flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-lg text-[10px] font-bold text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 transition-colors"
          >
            CANCEL
          </button>
        </div>
