...
function App() {
  ...
  useEffect(() => {
    ...
    EventsOn("traffic", (jsonStr: string) => {
      try {
        setTraffic(JSON.parse(jsonStr));
      } catch (e) {}
    });
    ...
    try {
      ...
      if (uiList.length > 0 && s.last_profile_id) {
        if (s.last_profile_id.startsWith("mixed://")) {
          ...
          const exists = uiList.find(
            (p: UIProfile) => p.id === s.last_profile_id,
          );
  ...
  return (
    <div className="h-screen w-full flex bg-[#09090b] font-sans text-white overflow-hidden relative selection:bg-purple-500/30">
      ...
      <div className="flex-1 flex flex-col relative">
        <div
          className="h-10 flex justify-end items-center px-4 gap-3 bg-[#09090b] z-50 relative"
        ...
        <div className="flex-1 p-8 flex items-center justify-center overflow-hidden relative">
          <div
            className={`absolute top-4 left-1/2 -translate-x-1/2 z-50 transition-all duration-300 ${errorMsg ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4 pointer-events-none"}`}
          >
