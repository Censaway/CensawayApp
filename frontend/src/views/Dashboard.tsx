...
export const Dashboard: React.FC = () => {
  ...
  const getPingColor = (ms?: number) => {
    if (ms === undefined) return "bg-gray-600";
    if (ms === -1 || ms >= 999) return "bg-red-500";
  ...
  const checkPings = async (e: React.MouseEvent) => {
    ...
    for (let i = 0; i < updatedList.length; i++)
      updatedList[i].latency = undefined;
  ...
  const handleOpenRouting = async (e: React.MouseEvent, profileId: string) => {
    e.stopPropagation();
  ...
  const getSubProfiles = (subId: string) =>
    profiles.filter((p: UIProfile) => p.subscription_id === subId);
  ...
  const renderProfileItem = (profile: UIProfile) => (
    <div
      ...
      className={`group relative p-3 rounded-xl border transition-all cursor-pointer flex items-center justify-between outline-none ring-0 select-none mb-2 ${selectedId === profile.id ? "bg-purple-500/10 border-purple-500/40" : "bg-transparent border-transparent hover:bg-white/5"} ${isRunning && selectedId !== profile.id ? "opacity-50" : ""}`}
    >
      <div className="flex items-center gap-3 relative z-10 w-full min-w-0 pr-2">
        <div
          className={`w-2 h-2 shrink-0 rounded-full transition-all duration-300 ${getPingColor(profile.latency)} ${selectedId === profile.id && !profile.latency ? "shadow-[0_0_8px_#c084fc]" : ""}`}
        ></div>
        <div className="flex flex-col flex-1 overflow-hidden">
          <div className="flex items-center gap-2 w-full">
            <span
              className={`text-xs font-medium truncate transition-colors ${selectedId === profile.id ? "text-white" : "text-gray-400"}`}
            >
              {profile.name}
            </span>
            {profile.latency !== undefined && (
              <span
                className={`text-[9px] font-mono px-1.5 py-0.5 rounded ${profile.latency < 200 ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400"}`}
              >
  ...
  return (
    <>
      <div className="flex w-full max-w-5xl h-[520px] gap-6 animate-[fadeIn_0.3s_ease-out]">
        <div
          className={`glass flex-1 rounded-3xl overflow-hidden p-8 flex flex-col justify-between relative transition-all duration-700 ${isRunning ? "border-green-500/30 shadow-[0_0_60px_-10px_rgba(34,197,94,0.15)]" : ""}`}
        >
          <div
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-6">
            <button
              ...
              className={`relative group w-40 h-40 rounded-full flex items-center justify-center outline-none transition-all duration-500 ${isRunning ? "bg-green-500/10 shadow-[0_0_40px_rgba(34,197,94,0.2)]" : "bg-white/5 hover:bg-white/10 shadow-[0_0_40px_rgba(168,85,247,0.1)]"}`}
            >
              {isLoading && (
                <div className="absolute inset-[-4px] rounded-full border-2 border-transparent border-t-purple-500 border-r-purple-500/50 animate-spin z-0 pointer-events-none"></div>
              )}
              <div
                className={`absolute inset-0 rounded-full border-2 transition-all duration-500 z-10 ${isRunning ? "border-green-500/50 scale-100" : "border-purple-500/30 group-hover:border-purple-400/50 scale-95"}`}
              ></div>
              <div className="z-20">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className={`h-16 w-16 transition-all duration-500 ${isRunning ? "text-green-400 drop-shadow-[0_0_10px_rgba(74,222,128,0.8)]" : "text-gray-400 group-hover:text-white"}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
            ...
            <div className="text-center w-full">
              ...
              <div className="h-8 flex items-center justify-center mt-2 mb-4">
                {isRunning && currentIp && (
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 shadow-[0_0_15px_-3px_rgba(16,185,129,0.3)] animate-[scaleIn_0.3s_ease-out]">
                    ...
                    <span className="text-xs font-mono font-bold text-emerald-100 tracking-wide">
                      {maskIp(currentIp)}
                    </span>
                  </div>
                )}
              ...
              <div
                className={`flex justify-center gap-3 max-w-[320px] mx-auto transition-all duration-500 ${isRunning ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
              >
                <div className="bg-black/20 rounded-xl p-3 border border-white/5 flex-1 flex flex-col items-center justify-center min-w-[120px]">
                  <div className="text-[9px] text-gray-500 uppercase mb-1 flex items-center gap-1.5 font-bold tracking-wider">
                    <svg
                      className="w-3 h-3 text-emerald-400"
                      fill="none"
                      viewBox="0 0 24 24"
        ...
        <div className="glass w-80 rounded-3xl p-6 flex flex-col relative">
          <div className="flex justify-between items-center pb-4 border-b border-white/5 mb-5">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={checkPings}
                className={`w-6 h-6 rounded flex items-center justify-center transition-all bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white`}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className={`h-3.5 w-3.5 ${isPinging ? "animate-spin text-purple-400" : ""}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              <button
                ...
                className={`w-6 h-6 rounded flex items-center justify-center transition-all duration-500 ${isAdding ? "bg-purple-600 text-white rotate-45" : "bg-white/5 hover:bg-white/10 text-gray-400"}`}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
          ...
          >
            <div className="overflow-hidden">
              <div
                className={`pb-4 transition-all duration-500 ${isAdding ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2"}`}
              >
                <div className="bg-black/20 p-3 rounded-xl border border-white/5">
                  <div className="flex gap-2 mb-2">
                    <button
                      onClick={() => setAddType("key")}
                      className={`flex-1 text-[10px] py-1 rounded transition-colors ${addType === "key" ? "bg-white/10 text-white" : "text-gray-500 hover:text-gray-300"}`}
                    >
                      KEY
                    </button>
                    <button
                      onClick={() => setAddType("sub")}
                      className={`flex-1 text-[10px] py-1 rounded transition-colors ${addType === "sub" ? "bg-white/10 text-white" : "text-gray-500 hover:text-gray-300"}`}
                    >
                      SUB
                    </button>
                  </div>
            ...
            {subscriptions.map((sub) => (
              <div key={sub.id} className="mb-4 animate-slideIn">
                <div className="flex justify-between items-center mb-2 px-1 group">
                  <div className="text-[9px] font-bold text-purple-400/70 uppercase truncate max-w-[120px]">
                    {sub.name}
                  </div>
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={(e) => handleUpdateSub(e, sub.id)}
                      className="text-gray-500 hover:text-white p-1"
                      title="Update"
                    >
                      <svg
                        className={`w-3 h-3 ${updatingSubId === sub.id ? "animate-spin" : ""}`}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                    ...
                    <button
                      onClick={(e) => requestDeleteSub(e, sub.id)}
                 ...
                 <div className="space-y-2 max-h-60 overflow-y-auto pr-1 scrollbar-thin">
                    ...
                    {availableServers.map(s => (
                        <button key={s.tag} onClick={() => applyRouting(s.tag)} className="w-full text-left p-3 rounded-xl bg-white/5 hover:bg-white/10 text-xs text-gray-200 flex justify-between items-center transition-colors group">
                            <span>{s.name}</span>
                            <span className="text-[9px] font-bold uppercase bg-black/40 px-1.5 py-0.5 rounded text-gray-500 group-hover:text-white transition-colors">{s.type}</span>
                        </button>
                    ))}
                 </div>
              </div>
