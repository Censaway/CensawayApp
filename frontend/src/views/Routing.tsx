...
export const RoutingView: React.FC = () => {
    ...
    const deleteRule = (id: string) => {
        const updatedRules = settings.user_rules.filter((r: main.UserRule) => r.id !== id);
        update({ user_rules: updatedRules });
    };
    ...
    return (
        <div className="w-full max-w-4xl animate-[fadeIn_0.3s_ease-out] flex gap-6 h-[520px]">
            <div className="glass flex-1 rounded-3xl p-8 border-t border-white/10 flex flex-col">
                ...
                <div className="flex items-center gap-3 mb-6 p-1 z-20 relative">
                    ...
                    <div className="flex-1 relative flex items-center">
                        <input type="text" placeholder={getPlaceholder()} value={newRule.value} onChange={(e) => setNewRule(new main.UserRule({...newRule, value: e.target.value}))} className="w-full h-10 bg-[#0a0a0e] border border-white/10 hover:border-purple-500/50 rounded-lg px-4 text-xs text-white placeholder:text-gray-600 outline-none focus:border-purple-500 focus:bg-[#121216] transition-all font-mono shadow-sm" />
                        {newRule.type === "process" && ( <button onClick={() => setIsProcessModalOpen(true)} className="absolute right-1 top-1 bottom-1 px-3 bg-white/10 hover:bg-white/20 text-[9px] font-bold text-gray-300 rounded-md transition-colors">LIST</button> )}
                    </div>
            ...
            <ProcessSelectorModal isOpen={isProcessModalOpen} onClose={() => setIsProcessModalOpen(false)} onSelect={(name) => setNewRule(new main.UserRule({...newRule, value: name}))} />
        </div>
    );
};