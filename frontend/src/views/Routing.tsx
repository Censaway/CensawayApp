...
export const RoutingView: React.FC = () => {
  ...
  const addRule = () => {
    ...
    const ruleToAdd = new main.UserRule({
      ...newRule,
      id: Date.now().toString(),
    });
    const updatedRules = [...settings.user_rules, ruleToAdd];
    update({ user_rules: updatedRules });
    setNewRule(
      new main.UserRule({
        id: "",
        type: "domain",
  ...
  const deleteRule = (id: string) => {
    const updatedRules = settings.user_rules.filter(
      (r: main.UserRule) => r.id !== id,
    );
    update({ user_rules: updatedRules });
  };

  const saveRuDomains = () => {
    const domains = ruDomainsText
      .split("\n")
      .map((s) => s.trim())
      .filter((s) => s !== "");
    update({ ru_domains: domains });
  };
    ...
    <div className="w-full max-w-4xl animate-[fadeIn_0.3s_ease-out] flex gap-6 h-[520px]">
      <div className="glass flex-1 rounded-3xl p-8 border-t border-white/10 flex flex-col">
        ...
        <div className="flex items-center gap-3 mb-6 p-1 z-20 relative">
          <CustomSelect
            value={newRule.type}
            onChange={(v) =>
              setNewRule(new main.UserRule({ ...newRule, type: v }))
            }
            options={ruleTypes}
          />
          <div className="flex-1 relative flex items-center">
            <input
              ...
              onChange={(e) =>
                setNewRule(
                  new main.UserRule({ ...newRule, value: e.target.value }),
                )
              }
          ...
          <CustomSelect
            ...
            onChange={(v) =>
              setNewRule(new main.UserRule({ ...newRule, outbound: v }))
            }
            options={[
              { value: "direct", label: "Direct", color: "text-emerald-400" },
              { value: "proxy", label: "Proxy", color: "text-purple-400" },
          ...
          <button
            ...
            className="h-10 w-10 shrink-0 flex items-center justify-center bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white rounded-lg transition-all shadow-[0_0_15px_rgba(168,85,247,0.3)] hover:shadow-[0_0_25px_rgba(168,85,247,0.5)] active:scale-95"
          >
            <svg
        ...
        <div className="flex-1 overflow-y-auto space-y-1 mt-2 pr-1 scrollbar-hide z-0">
          {settings.user_rules.length === 0 ? (
            ...
            settings.user_rules.map((rule: main.UserRule) => (
              <div
                key={rule.id}
                className="grid grid-cols-12 gap-4 items-center bg-white/5 hover:bg-white/10 p-3 rounded-lg border border-transparent hover:border-white/5 transition-colors group animate-slideIn"
              >
                <div className="col-span-2">
                  <span
                    className={`px-2 py-1 rounded text-[9px] font-bold uppercase tracking-wider ${rule.outbound === "direct" ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/20" : rule.outbound === "proxy" ? "bg-purple-500/20 text-purple-400 border border-purple-500/20" : "bg-red-500/20 text-red-400 border border-red-500/20"}`}
                  >
                    {rule.outbound}
                  </span>
                </div>
                <div className="col-span-2 text-[10px] text-gray-400 uppercase font-bold">
                  {rule.type}
                </div>
                <div className="col-span-7 text-xs text-gray-200 font-mono truncate select-text">
                  {rule.value}
                </div>
      ...
      <div className="glass w-64 rounded-3xl p-6 border-t border-white/10 flex flex-col">
        ...
        <div className="flex-1 relative mb-4">
          <textarea
            value={ruDomainsText}
            onChange={(e) => setRuDomainsText(e.target.value)}
            className="w-full h-full bg-[#0a0a0e] border border-white/10 rounded-xl p-3 text-[10px] font-mono text-gray-300 outline-none focus:border-purple-500/50 resize-none scrollbar-hide leading-relaxed"
            placeholder=".ru&#10;yandex.ru&#10;..."
          />
        </div>
        <button
      ...
      <ProcessSelectorModal
        ...
        onSelect={(name) =>
          setNewRule(new main.UserRule({ ...newRule, value: name }))
        }
      />
    </div>
  );
