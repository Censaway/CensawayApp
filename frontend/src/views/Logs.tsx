...
const LogLine = React.memo(({ log, index }: { log: string; index: number }) => {
  ...
  } else if (log.includes("FATAL") || log.includes("panic")) {
  ...
  if (match && match.index !== undefined) {
    ...
    return (
      <div
        className={`flex gap-2 px-2 py-0.5 rounded text-[10px] font-mono leading-relaxed hover:bg-white/5 ${bgClass}`}
      >
        <span className="text-gray-600 select-none w-8 text-right shrink-0">
          {index + 1}
        </span>
        <span className="text-gray-500 shrink-0 whitespace-pre">
          {timePart}
        </span>
        <span className={`${levelClass} font-bold shrink-0 w-10`}>
          {levelPart}
        </span>
...
export const LogsView: React.FC = () => {
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter((log: string) => {
      ...
      if (search && !log.toLowerCase().includes(search.toLowerCase()))
        return false;
      return true;
    });
  ...
  return (
    <div className="w-full max-w-5xl h-[520px] glass rounded-3xl overflow-hidden flex flex-col border-t border-white/10 animate-[fadeIn_0.3s_ease-out]">
      ...
      <div
        ref={containerRef}
        className="flex-1 min-h-0 overflow-y-auto p-2 bg-[#0a0a0e]/50 select-text scrollbar-thin z-10"
      >
