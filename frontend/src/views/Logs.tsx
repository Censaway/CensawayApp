            ...
            <div className="flex justify-between items-center px-4 py-3 bg-black/20 border-b border-white/5 gap-4 z-20">
                <div className="flex items-center gap-3 flex-1">
                    <div className="relative flex-1 max-w-xs">
                        ...
                        <input type="text" placeholder="Search logs..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-[#0a0a0e] border border-white/10 rounded-lg pl-9 pr-3 py-2 text-[10px] text-gray-200 focus:border-purple-500/50 outline-none transition-colors shadow-sm" />
                    </div>
                    <CustomSelect value={filterLevel} onChange={setFilterLevel} options={filterOptions} />
                </div>
                <div className="flex items-center gap-2">
                    <button onClick={() => setAutoScroll(!autoScroll)} className={`p-2 rounded-lg transition-all border border-transparent ${autoScroll ? "text-purple-400 bg-purple-500/10 border-purple-500/20" : "text-gray-500 hover:text-gray-300 hover:bg-white/5"}`} title={autoScroll ? "Auto-scroll ON" : "Auto-scroll OFF"}><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" /></svg></button>
